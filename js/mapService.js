class MapService {
    ...
    createPopupContent(location, details) {
        ...
        const formatTargetType = (targetType) => {
            if (!targetType || targetType === "No Info" || targetType === "Məlumat yoxdur") return "No Info";
            
            if (targetType.toLowerCase().includes('military') || 
                targetType.toLowerCase().includes('base') ||
                targetType.toLowerCase().includes('army') ||
                targetType.toLowerCase().includes('defense')) {
                return `Military Area: ${targetType}`;
            }
            
            if (targetType.toLowerCase().includes('nuclear') || 
                targetType.toLowerCase().includes('atomic') ||
                targetType.toLowerCase().includes('uranium')) {
                return `Nuclear Facility: ${targetType}`;
            }
            
            if (targetType.toLowerCase().includes('residential') || 
                targetType.toLowerCase().includes('civilian') ||
                targetType.toLowerCase().includes('city') ||
                targetType.toLowerCase().includes('urban')) {
                return `Civilian Area: ${targetType}`;
            }
            
            if (targetType.toLowerCase().includes('airport') || 
                targetType.toLowerCase().includes('port') ||
                targetType.toLowerCase().includes('station') ||
                targetType.toLowerCase().includes('facility')) {
                return `Infrastructure: ${targetType}`;
            }
        ...
        return `
            ...
            <div class="incident-body dark-mode">
                <div class="target-info">
                    ...
                    <div class="specific-target">${formatTargetType(location.targetType)}</div>
                </div>
                
                <div class="attack-stats">
                    <div class="stat-box" style="grid-column: 1 / -1;">
                        <div class="label">Attacker</div>
                        <div class="value">${getCountryName(location.attacker)}</div>
                    </div>
                </div>
                ...
                ${location.casualties ? `
                    <div class="casualties-grid">
                        <div class="stat-box">
                            ...
                            <div class="value">${location.casualties.dead || 0}</div>
                        </div>
                        <div class="stat-box">
                            <div class="label">Wounded</div>
                            <div class="value">${location.casualties.wounded || 0}</div>
                        </div>
                    </div>
                ` : ''}
            </div>
            <div class="incident-footer dark-mode">
                <div class="incident-meta">
                    <span class="coordinates">${location.lat.toFixed(4)}°N, ${location.lon.toFixed(4)}°E</span>
                </div>
            </div>
        `;
    }
