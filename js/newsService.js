class NewsService {
    constructor(mapService) {
        ...
        // Setup clear manual button handler
        const clearManualButton = document.getElementById('clear-manual');
        if (clearManualButton) {
            clearManualButton.addEventListener('click', () => this.handleClearManual());
        }
    }
    ...
    addNewsItemToFeed(id, item) {
        try {
            ...
            this.mapService.addIncident(id + '-' + location.name, {
                ...location,
                id: id // Ensure ID is passed to the marker
            }, {
                title: item.title,
    ...
    async handleClearManual() {
        try {
            if (!confirm('Bütün manual əlavə edilmiş markerləri silmək istədiyinizə əminsiniz?')) {
                return;
            }

            console.log('Clearing all manual markers...');
            const clearButton = document.getElementById('clear-manual');
            clearButton.disabled = true;
            clearButton.textContent = 'Silinir...';

            // Remove all manual entries from the map
            this.newsItems.forEach((item, id) => {
                if (id.startsWith('manual-')) {
                    // Remove from map
                    this.mapService.removeIncident(id + '-' + item.locations[0].name);
                    // Remove from newsItems
                    this.newsItems.delete(id);
                }
            });

            // Remove all manual entries from Firebase
            const newsRef = FirebaseService.ref(FirebaseService.database, 'news');
            const snapshot = await FirebaseService.get(FirebaseService.child(newsRef, '/'));
            ...
            if (snapshot.exists()) {
                ...
                // Update Firebase to remove manual entries
                await FirebaseService.set(newsRef, updates);
            }

            console.log('All manual markers cleared successfully');
            clearButton.textContent = 'Bütün Manual Markerləri Sil';
            clearButton.disabled = false;
            alert('Bütün manual markerlər silindi!');

        } catch (error) {
            console.error('Error clearing manual markers:', error);
